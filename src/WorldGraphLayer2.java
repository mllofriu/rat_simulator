...
public abstract class WorldGraphLayer2 extends NslModule {
	...
	public WorldGraphLayer2(String nslName, NslModule nslParent) {
	...
	public void simRun() {
		...
		if (HasselmoGridCellLayer.cantActivas>0) {
		...
		map_activation(currentDirection, Rat.newTrial||(lastGridAct ==0), newNode, Drive.ateFood);
	...
	private NodePattern getNearestPattern2(double[] patron, int currentDir) {
		...
		if ((!(Rat.newTrial||(lastGridAct ==0))) && (currentDir != previous_direction)) { // Si el
																		// nodo
																		// no
																		// cambio
																		// busco
			...
			for (mapIterator = map_list.listIterator(); mapIterator.hasNext();) {
				...
				if ((mapNode.getId() != nodosVisitados.getLast())) {
...
//			}
	...
	private NodePattern getPatternInDir(NodeMap mapNode, int direction) {
		...
		for (ListIterator<NodePattern> patIterator = pattern_list
			...
			if ((mapNode.getId() == patNode.mapNodeID) && (patNode.dir == direction)) {
				result = patNode;
				break;
			}
		}
...
//			if (!(Rat.newTrial||(lastGridAct ==0))) {
			if ((!Rat.newTrial)&&(!nodosVisitados.isEmpty())) {
				...
				if ((patWinner.mapNodeID != idUltimoNodo)
					...
					nodosVisitados.addLast(nodeWinner.getId());
					cambioNodoActual = true;
					currentNode = nodeWinner.getId();
					// } else {
					// linkNodes=false;
					// }
					// .err
	...
	private double patternNodeDistance(NodeMap a, NodeMap b) {
		...
		for (patIterator = pattern_list.listIterator(); patIterator.hasNext();) {
			...
			if (patNodeIter.mapNodeID == a.getId())
				patronesA[patNodeIter.dir] = patNodeIter;
			else if (patNodeIter.mapNodeID == b.getId())
	...
	private void mergeNodes(NodeMap mapNodeI, NodeMap mapNodeJ) {
		...
		combinarPatrones(mapNodeI.getId(), mapNodeJ.getId());
		...
		if (currentNode == mapNodeJ.getId()) {
			currentNode = mapNodeI.getId();
		}
		...
		*/
		...
		for (int iterNodos = 0; iterNodos < nodosVisitados.size(); iterNodos++) {
			if (nodosVisitados.get(iterNodos) == mapNodeJ.getId()) {
				nodosVisitados.remove(iterNodos);
				nodosVisitados.add(iterNodos, mapNodeJ.getId());
			}
		}
	}
	...
	private void map_activation(int currentDir, boolean newTrial,
		...
		if (map_list.isEmpty()) {
			...
			patNode.mapNodeID = newMapNode.getId();
	...
	private void eliminarPatronesNodo(NodeMap mapNode) {
		...
		for (patIterator = pattern_list.listIterator(); patIterator.hasNext();) {
			pattern = patIterator.next();
			if (pattern.mapNodeID == mapNode.getId()) {
	...
	private void deleteNode(NodeMap mapNode) {
		...
		for (mapIterator = map_list.listIterator(); mapIterator.hasNext();) {
			...
			for (int iterEdge = 0; iterEdge < sourceMapNode.pointerWGtoWG
				...
				if (edge.points_to_node == mapNode.getId()) {
