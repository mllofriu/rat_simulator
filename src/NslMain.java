...
import nslj.src.cmd.NslCmdBreak;
import nslj.src.cmd.NslCmdBreakCycles;
import nslj.src.cmd.NslCmdBreakEpochs;
import nslj.src.cmd.NslCmdBreakModules;
import nslj.src.cmd.NslCmdCont;
import nslj.src.cmd.NslCmdContCycle;
import nslj.src.cmd.NslCmdContEpoch;
import nslj.src.cmd.NslCmdContModule;
import nslj.src.cmd.NslCmdDoRunEpochTimes;
import nslj.src.cmd.NslCmdDoTrainEpochTimes;
import nslj.src.cmd.NslCmdEndModule;
import nslj.src.cmd.NslCmdEndRun;
import nslj.src.cmd.NslCmdEndRunEpoch;
import nslj.src.cmd.NslCmdEndSys;
import nslj.src.cmd.NslCmdEndTrain;
import nslj.src.cmd.NslCmdEndTrainEpoch;
import nslj.src.cmd.NslCmdExit;
import nslj.src.cmd.NslCmdInitModule;
import nslj.src.cmd.NslCmdInitRun;
import nslj.src.cmd.NslCmdInitRunEpoch;
import nslj.src.cmd.NslCmdInitSys;
import nslj.src.cmd.NslCmdInitTrain;
import nslj.src.cmd.NslCmdInitTrainEpoch;
import nslj.src.cmd.NslCmdRun;
import nslj.src.cmd.NslCmdSimRun;
import nslj.src.cmd.NslCmdSimTrain;
import nslj.src.cmd.NslCmdStepCycle;
import nslj.src.cmd.NslCmdStepEpoch;
import nslj.src.cmd.NslCmdStepModule;
import nslj.src.cmd.NslCmdTrain;
import nslj.src.cmd.NslCmdTrainAndRunAll;
...
/**
 * The main routine. Parses the command line options, instantiates all essential
 * components in the simulator, launches the interpreter and loads the model.
 * 
 * The command line options are:
 * 
 * -schedulerMethod [Select the scheduling method -noDisplay [Determines if the
 * graphic interface should be used] -stdout & -stderr [Select the display where
 * the messages should be displayed] -debug [Determines if the debug messages
...
public class NslMain {
	 ...
	 * 
	 * @param argv
	 *            The vector containing the command line arguments.
	 * @throws NslParseArgumentException
	 *             This exception is thrown if the user does not provide enough
	 *             or correct information in the command line.
	 */
	...
	private static void parseArguments(String argv[])
		...
		for (int i = 1; i < argv.length; i++) {
			...
			if (argv[i].compareTo("-stdout") == 0) {
				...
				if (i == argv.length) {
					System.err
							.println("-debug requires a positive numeric level");
					throw new NslParseArgumentException();
				}
				i++;
				try {
	...
	/**
	 * Add all differential / numerical methods to the system. Setup the default
	 * numerical methods to run.
	 * 
	...
	/**
	 * Add the model to the system. All the corresponding children modules are
	 * instantiated and registered in the schduler.
	 * 
	 * @todo Make the system flexible, so it can load dynamically any compiled
	 *       model.
	 */
	...
	private static void loadModel(String s) {
		...
		try {
			...
			System.err.println("NslMain [Error]: The model \"" + modelname
					+ "\" was not found in the classpath");
			System.err
					.println("NslMain [Suggestion]: Check for possible misspelling in the name of the model");

		} catch (ExceptionInInitializerError initializerError) {

			System.err
					.println("NslMain [Error]: Generated while initializing \""
							+ modelname + ".class\"");

		} catch (LinkageError linkageError) {

			System.err
					.println("NslMain [Error]: Class linkage failed while loading \""
							+ modelname + ".class\"");
			System.err
					.println("NslMain [Suggestion]: Recompile the model and/or the nsl simulator");

		} catch (InternalError internalError) {

			System.err
					.println("NslMain [Error]: An unexpected internal error has occurred in the Java VM");
			System.err.println("NslMain [Error]: "
					+ internalError.toString().substring(
							internalError.toString().indexOf(":") + 2));

			System.exit(0);

		} catch (Exception e) {

			System.err.println("NslMain [Error]: \"" + e.toString()
					+ "\"  when loading/constructing model " + modelname
					+ ".class");
			if (debug > 0) {
				e.printStackTrace();
			}
	...
	/**
	 * Main routine for NslMain application. It instantiates essential
	 * components in the simulator and launch the interpreter for user input. It
	 * also adds user commands, differential methods and modules into the
	 * simulator.
	 * 
	 * @param argv
	 *            The vector containing the command line arguments.
	...
	public static void main(String argv[]) {
		...
		try {
			...
			System.err.println();
			System.err
					.println("Usage:   java NslMain \"ModelName\" [-options]");
			System.err.println();
